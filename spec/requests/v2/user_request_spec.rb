...
RSpec.describe 'users API' do
  ...
  it 'returns the logged in users attributes and relationships' do
    ...
    expect(user_info[:data][:id]).to eq("#{@user.id}")
  ...
  it 'returns an error if not enough information is provided to create a user' do
    body = {
      email: "googlydoogle@gmizzledizzle.com",
      first_name: "Alec",
      last_name: "Baldwin",
  ...
  it 'returns an error if no user is logged-in' do
    delete "/api/v1/sessions", headers: @headers_1
    ...
    expect(user_info[:error]).to eq('Please log-in')
  end
end
